...
const customLayerConstruct = (i, _layers = []) => {
  ...
  let mappedDnaToLayers = layers.map(layer => {
    let selectedElement, selectedElementArray;
    if (layer.name.includes('Skin Tone')) {
      selectedElement = layer.elements.find(el => el.name == skin)
    } else {
      selectedElementArray = layer.elements.filter(el => traits.includes(el.name));
      selectedElement = selectedElementArray.length > 0 ? selectedElementArray[0] : null;
    }
  ...
  mappedDnaToLayers = mappedDnaToLayers.filter(n => n);
  return mappedDnaToLayers;
};
...
const startCreating = async () => {
  ...
  let single = layerConfigurations[layerConfigIndex].single;
  for (
    let i = 0;
  ...
  while (layerConfigIndex < layerConfigurations.length) {
    ...
    for (var i = start; i < end; i++) {
      ...
      if (true) {
        let results = customLayerConstruct(i, layers);
        let loadedElements = [];
